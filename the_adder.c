...
// WARNING: Removing unreachable block (ram,0x001007f7)
// WARNING: Removing unreachable block (ram,0x00100803)
...

{
  ...
  char choice;
  uint i;
  ulonglong sum_table [10];
  
  lVar1 = *(long *)(in_FS_OFFSET + 0x28);
  puts("I can sum any value you provide me. Wanna try?");
  memset(sum_table,0,80);
  i = 1;
  while( true ) {
    while( true ) {
      ...
      puts("3) Quit");
      printf("> ");
      __isoc99_scanf(&DAT_00100d27,&choice);
      if (choice != '2') break;
      print_history(sum_table,i);
    }
    if (choice == '3') break;
    if (choice == '1') {
      _Var2 = add_value(sum_table,i);
      if (_Var2) {
        i = i + 1;
      }
    }
...
_Bool add_value(ulonglong *sum_table,uint i)

{
  ulonglong uVar1;
  long in_FS_OFFSET;
  uint i_local;
  ulonglong *sum_table_local;
  char choice;
  long local_10;
  
  local_10 = *(long *)(in_FS_OFFSET + 0x28);
  printf("Number: ");
  __isoc99_scanf(&DAT_00100d4b,sum_table + i);
  uVar1 = sum_table[i];
  if (uVar1 != 0) {
    printf("Are you sure you want me to add %llu? [y/n]\n",sum_table[i]);
    __isoc99_scanf(&DAT_00100d27,&choice);
    if (choice == 'y') {
      sum_table[i] = sum_table[i - 1] + sum_table[i];
      uVar1 = 1;
    }
    else {
      sum_table[i] = 0;
...
void print_history(ulonglong *sum_table,uint i)
